...
// const streamingUrl = 'https://saveload.vittaverse.com/api/tradingview/chart/streaming_test'
const streamingUrl = 'https://ox-subtle-lark.ngrok-free.app/api/tradingview/chart/streaming'
...
function handleStreamingData(data) {
  ...
  const options = {
    ...
    second:'2-digit',
    hour12: false,
    timeZone: 'UTC' // Set the time zone to UTC to ensure consistency
  };
  
  const resolution = subscriptionItem.resolution; // Assuming it's a string like '1D', '1W', etc.
  const lastDailyBar = subscriptionItem.lastDailyBar;
  const nextDailyBarTime = getNextDailyBarTime(lastDailyBar.time);
  const formatter = new Intl.DateTimeFormat('en-US', options);
  
  const lastDailyBarFormatted = formatter.format(lastDailyBar.time);
  const nextDailyBarTimeFormatted = formatter.format(nextDailyBarTime);
  ...
  const getLastDailyBarTimestamp = (formattedTime) => {
    ...
    currentDate.setUTCHours(hours, minutes, seconds, 0); // Set UTC hours
    return currentDate.getTime();
  };
  
  let resolutionInMinutes = 0;
  ...
  if (resolution === '1D') {
    ...
    resolutionInMinutes = parseInt(resolution); // Use the original resolution value
  }
  ...
  const adjustedNextDailyBarTimestamp = nextDailyBarTimestamp - 2 * 60 * 60 * 1000;
